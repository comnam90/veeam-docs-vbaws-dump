...
By default, Veeam Backup for AWS deploys worker instances used to perform restore operations in the [backup account](worker_options.md). However, you can instruct Veeam Backup for AWS to deploy worker instances in a production account — that is, an account to which the volumes will be restored. To do that, set the Deploy workers in production account toggle to On.
