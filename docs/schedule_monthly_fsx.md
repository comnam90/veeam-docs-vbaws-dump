...
| --- |
| Note |
| Veeam Backup for AWS does not create backup copies independently from FSx backups. That is why when you select hours for backup copies, the same hours are automatically selected for backups. To learn how Veeam Backup for AWS performs backup, see [FSx Backup](backup_hiw_fsx.md). |
