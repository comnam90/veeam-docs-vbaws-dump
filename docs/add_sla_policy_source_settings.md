...
The list of available regions will depend on the option selected at [step 3](add_sla_policy_scope.md) of the wizard. If you have selected the Organization option, the list will contain all existing AWS Regions; if you have selected the Account option, the list will contain all AWS Regions enabled for the AWS account.

1. To save changes made to the backup policy settings, click Apply.

[![Creating SLA-Based EC2 Policy](images/add_sla_policy_regions.webp)](images/add_sla_policy_regions.webp "Creating SLA-Based EC2 Policy")
...
[![Creating SLA-Based EC2 Policy](images/add_sla_policy_resources.webp)](images/add_sla_policy_resources.webp "Creating SLA-Based EC2 Policy")
...
For a resource to be displayed in the list of available resources, it must reside in an AWS Region that has ever been specified in any backup policy. Consider that the list will display resources only if the region has ever been specified in any backup policy. Otherwise, the only option to discover the resources is to click Browse to select specific resources from the global list and to wait for Veeam Backup for AWS to rescan the region and to populate the resource list.
