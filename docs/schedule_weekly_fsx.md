---
title: "schedule_weekly_fsx"
source_url: "https://helpcenter.veeam.com/docs/vbaws/guide/schedule_weekly_fsx.html"
...
| --- |
| Note |
| Veeam Backup for AWS does not create backup copies independently from file system backups. That is why when you select days to create backup copies, the same days are automatically selected for backups. To learn how Veeam Backup for AWS performs backup, see [FSx Backup](backup_hiw_fsx.md). |
...
If a restore point is older than the specified time limit, Veeam Backup for AWS removes the restore point from the chain. For more information, see [FSx Backup Retention](retention_backup_fsx.md).
...
[![Creating FSx Backup Policy](images/schedule_weekly_fsx.webp)](images/schedule_weekly_fsx.webp "Creating FSx Backup Policy")
